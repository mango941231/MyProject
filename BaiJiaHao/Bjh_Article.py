"""抓取百家号文章信息 代码28-31把页面不存在的账号存储在txt文档"""
import requests
...
class Bjharticle:
    ...
    def article(self):
        try:
            ...
            print('%s链接解析出现错误' % self.Id)
        try:
            if artjson['html'] == "   ":
                #出现“页面不存在或已被删除”情况则把该id存储在txt文档
                filename = 'F:\workspace\baijiahaofail.txt'  #存储路径
                with open(filename, 'a') as f:
                    f.write(self.Id + '\n')
                print('%s 页面不存在或已被删除' % self.Id)
            else:
                try:
                ...
                try:
                    for item in items:
                        ...
                        PostThumbnail = item['cover_images']  # 封面图
                    ...
                    print('%s文章解析出现错误' % self.Id)
        except:
            print('%s判断页面为空出现错误' % self.Id)
